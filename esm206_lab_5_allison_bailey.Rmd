...
-  Find confidence intervals & do t-tests wth t.test()
...
ggplot(data = lobster_tidy, aes(sample = size_mm)) +
  geom_qq() +
  facet_wrap(~site, scales = "free", (size = 0.5))
```
...
Use the 't.text()' function to find confidence intervals and perform t-tests
...
Two-sample t-test: Is there a ignificant difference in mean lobster lengths between Naples Reef and Mohawk Reef?
```{r}
